...
def get_articles_url_for_page(page_num):
    BASE_URL = 'https://newswithviews.com/page/'
    url_for_date = BASE_URL + str(page_num)
    resp = requests.get(url_for_date)
    soup = BeautifulSoup(resp.text, 'lxml')
    article_url_tags = soup.find("div", {"class": "fusion-blog-shortcode-1"}).find("div").findAll("h2", {"class": "entry-title"})
...
def add_articles_for_page_to_database(page_num, max_number_to_add=10000):
    ...
    if not Publication.query.filter_by(name = 'newswithviews.com').first():
        Publication.create_publication(name = 'newswithviews.com')
